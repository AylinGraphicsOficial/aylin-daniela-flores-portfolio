...
export const ProjectEditModal: React.FC<ProjectEditModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const finalProject: Project = {
      ...
      client: formData.client || 'Cliente Especial',
      ...
      tags: tagsArray.length > 0 ? tagsArray : ['Design', 'Kinetic'],
  ...
  const bgModal = darkMode ? 'bg-[#081208] text-white border-white/10' : 'bg-white text-gray-900 border-gray-200';
  const bgInput = darkMode ? 'bg-white/5 border-white/10 text-white placeholder-gray-500 focus:border-[#76FF03]' : 'bg-gray-50 border-gray-200 text-gray-900 placeholder-gray-400 focus:border-emerald-500';
  const textMuted = darkMode ? 'text-gray-400' : 'text-gray-500';

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fade-in overflow-y-auto">
      <div
        className={`relative w-full max-w-4xl max-h-[90vh] overflow-y-auto ${bgModal} border rounded-3xl p-6 sm:p-8 shadow-2xl my-8`}
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between pb-4 mb-6 border-b border-white/10">
          <div>
            <span className="text-[10px] font-mono font-bold tracking-[0.25em] text-[#76FF03] uppercase block mb-1">
              EDITOR DE PROYECTO
            </span>
        ...
        {/* Suggestion & Format Guidance Banner */}
        <div className="mb-6 p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 flex items-start gap-3">
          ...
          <div className="text-xs space-y-1">
            <span className="font-bold text-emerald-300 block">
              💡 Guía de Formatos & Sugerencias Multimedia:
            </span>
            <p className={textMuted}>
              • <strong>Imágenes / Renders:</strong> Formatos WebP o PNG recomendados (1920×1080 px o 1200×800 px).
              <br />• <strong>Videos & Clips:</strong> Enlaces directos a YouTube/Vimeo o clips MP4/WebM ligeros (&lt; 20 MB).
              <br />• <strong>GIFs Animados:</strong> Archivos optimizados (&lt; 10 MB) para reproducción fluida.
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-2.5 rounded-xl border ${bgInput} outline-none text-sm transition-all`}
              />
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              >
                <Star className={`w-4 h-4 ${formData.featured ? 'fill-[#76FF03]' : ''}`} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <textarea
                ...
                placeholder="Resumen conciso del proyecto para tarjetas del catálogo..."
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Detalles técnicos, proceso de modelado, iluminación, texturizado e impacto..."
                className={`w-full px-4 py-2.5 rounded-xl border ${bgInput} outline-none text-xs leading-relaxed`}
              />
            </div>
          </div>
          ...
          <div className="p-4 rounded-2xl border border-white/10 bg-white/5 space-y-4">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4">
              {/* Preview */}
              <div className="w-28 h-20 rounded-xl overflow-hidden bg-black/40 border border-white/15 flex items-center justify-center flex-shrink-0">
                {formData.image ? (
              ...
              <div className="flex-1 w-full space-y-2">
                ...
                />
                <label className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/10 hover:bg-[#76FF03] hover:text-black transition-colors cursor-pointer text-xs font-bold font-mono">
          ...
          {/* Row 5: Videos, Clips & GIFs */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label className="text-xs font-mono font-bold uppercase tracking-wider flex items-center gap-1.5 mb-1.5">
                <Film className="w-3.5 h-3.5 text-[#76FF03]" />
                <span>Enlace de Video (YouTube / Vimeo)</span>
              </label>
              <input
                type="text"
            ...
            <div>
              <label className="text-xs font-mono font-bold uppercase tracking-wider flex items-center gap-1.5 mb-1.5">
                <Video className="w-3.5 h-3.5 text-[#76FF03]" />
          ...
          <div>
            ...
            <input
              ...
              placeholder="3D Modeling, Blender, Octane, Substance 3D, After Effects"
          ...
          <div className="p-4 rounded-2xl border border-white/10 bg-white/5 space-y-3">
            <div className="flex items-center justify-between">
              <label className="text-xs font-mono font-bold uppercase tracking-wider">
                Métricas & Resultados Clave ({formData.metrics?.length || 0})
              </label>
              <button
                type="button"
                onClick={handleAddMetric}
                className="text-xs text-[#76FF03] hover:underline flex items-center gap-1 font-bold"
              >
              ...
              {(formData.metrics || []).map((m, idx) => (
                <div key={idx} className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    className={`w-36 px-3 py-1.5 rounded-lg border ${bgInput} text-xs font-mono font-bold text-[#76FF03]`}
                  />
                  <button
                    type="button"
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-white/10">
            <button
              ...
              className="px-5 py-2.5 rounded-xl border border-white/10 hover:bg-white/10 text-xs font-bold tracking-wider uppercase transition-all"
            >
              Cancelar
            </button>
            <SpecularButton
              ...
              size="md"
              radius={12}
              className="text-xs font-black tracking-wider uppercase px-6 py-2.5 flex items-center space-x-2"
            >
