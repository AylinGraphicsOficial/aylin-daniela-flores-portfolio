...
const DEFAULT_CONFIG: Required<Omit<GradualBlurProps, 'preset' | 'hoverIntensity' | 'onAnimationComplete' | 'width' | 'mobileHeight' | 'tabletHeight' | 'desktopHeight' | 'mobileWidth' | 'tabletWidth' | 'desktopWidth'>> & { width?: string } = {
  position: 'bottom',
  strength: 2,
  height: '6rem',
  divCount: 5,
...
const useResponsiveDimension = (responsive: boolean, config: any, key: string) => {
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      if (w <= 480 && config[`mobile${capitalized}`]) v = config[`mobile${capitalized}`];
      else if (w <= 768 && config[`tablet${capitalized}`]) v = config[`tablet${capitalized}`];
      else if (w <= 1024 && config[`desktop${capitalized}`]) v = config[`desktop${capitalized}`];
      setValue(v);
    };
